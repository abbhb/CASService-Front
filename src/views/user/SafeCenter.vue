<template>
  <div id="changepassword" class="bos">
          ...
          <div class="profile-ability__list">
            ...
            <div class="ui-cell">
              ...
              <div class="ui-cell__bd">
                ...
                <div v-if="userInfo.email" class="ui-cell__info">{{ userInfo.email }}</div>
                <div v-else class="ui-cell__info">暂未绑定</div>
              </div>
            ...
            <div class="ui-cell">
              ...
              <div class="ui-cell__bd">
                <div class="ui-cell__title">MFA认证</div>
                <div class="ui-cell__info">使用GoogleAuthenticator身份认证</div>
              </div>
              <div class="ui-cell__ft" @click="googleAuthChange">
                <span v-if="googleAuth.state===0" class="btn-link">去绑定</span>
                <span v-else-if="googleAuth.state===1&&googleAuth.verify===0" class="btn-link">去验证</span>
                <span v-else-if="googleAuth.state===1&&googleAuth.verify===1" class="btn-link">关闭</span>
                <span v-else class="btn-link">去绑定</span>
              </div>
            </div>
          </div>
        ...
        <el-form ref="emailform" label-position="left" label-width="100px">
          <el-form-item v-if="this.emailform.origin_email" label="原邮箱验证码">
            ...
            <div class="nmlk" style="display: flex;flex-direction: row;">
              <el-input id="inputValue" v-model="emailform.origin_code" placeholder="请输入原邮箱验证码"
                        type="text"/>
          ...
          <el-form-item label="电子邮箱">
            <el-input v-model="emailform.new_email" placeholder="请输入电子邮箱" type="eamil"/>
          </el-form-item>
          <el-form-item label="邮箱验证码" prop="newpassword">
            <div style="display: flex;flex-direction: row;">
              <el-input id="inputValue" v-model="emailform.new_code" placeholder="请输入新邮箱验证码"
                        type="text"/>
    ...
    <el-dialog
        ...
        :close-on-press-escape="false"
        :show-close="true"
        :title="googleAuthDialog.title"
        :visible.sync="googleAuthDialog.dialogVisible"
        class="dialog"
        style="z-index: 2100"
        width="55%"
    >
      <el-card class="box-card">
        <div style="display: flex;flex-direction: row;">
          <div style="display: flex;flex-direction: column;align-items: center">
            <span>QRCode</span>
            <el-image :src="googleAuth.qrCode" style="width: 180px;height: 180px;"/>
          </div>

          <div style="display: flex;flex-direction: column;justify-content: center">
            <div>
              <span style="font-size: 28px;">秘钥 </span>
              <span>{{ googleAuth.secret }}</span>
            </div>
            <div style="height: 40%;">
              <el-tag>必须验证完成后才算启用完成!</el-tag>
            </div>
            <div style="display: flex;flex-direction: column">
              <span>动态密码</span>
              <div style="display: flex;flex-direction: row">
                <el-input v-model="googleAuthDialog.code"></el-input>
                <el-button style="margin-left: 5px;" type="primary" @click="googleAuthCommit">校验</el-button>
              </div>
...
import {changeState, getGoogleAuthenticator, verify} from "@/api/auth";
...
export default {
  ...
  created() {
    ...
    if (userInfo) {
      ...
      this.refreshGoogleAuth()
    }
  },
  methods: {
    async refreshGoogleAuth() {
      const res = await getGoogleAuthenticator()
    ...
    async googleAuthChange() {


      if (this.googleAuth.state === 0) {
        this.googleAuthDialog.title = "绑定MFA"
        const res = await changeState(1)
        if (String(res.code) === '1') {
        ...
        this.googleAuthDialog.dialogVisible = true
      } else if (this.googleAuth.state === 1 && this.googleAuth.verify === 0) {
        this.googleAuthDialog.title = "验证MFA"
        this.refreshGoogleAuth()
        this.googleAuthDialog.dialogVisible = true
      } else if (this.googleAuth.state === 1 && this.googleAuth.verify === 1) {
        const resd = await changeState(0)
    ...
    EmailWithUser() {
      ...
      console.log(this.classData.dialogVisible)
    },
    changepassword() {
      router.push({name: 'changepassword'})
    },
    async googleAuthCommit() {
      const res = await verify({
        //   空着即可
      }, this.googleAuthDialog.code);
      if (String(res.code) === '1') {
        this.$message.success(res.msg);
        this.googleAuthDialog.dialogVisible = false;
        this.refreshGoogleAuth()
      } else {
        this.$message.error(res.msg);
      }
